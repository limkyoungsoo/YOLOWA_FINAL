					...
					<div class="form-box" id="lForm">
						<div class="form-top">
							<div class="form-top-left">
								...
								<p><strong>YoloWa</strong> 페이지는 회원정보 수정이 가능합니다.</p>
							</div>
						...
						<div class="form-bottom">
							<form role="form" action="${pageContext.request.contextPath}/modifyMember.do" method="post"
								...
								<div class="form-group">
									<font style="font-size: 20px; color: white"> 아이디</font><input
										...
										class="form-username form-control" id="form-username" readonly="readonly">
								...
								<c:set var="checkPhone" value="${user.phone}" />
                     			<c:set var="phone" value="${fn:split(checkPhone,'-')}" />
								<div class="form-group">
									...
									<input type="text" id="phone2" value="${phone[1]}" class="form-username phoneInput2">
									<input type="text" id="phone3" value="${phone[2]}" class="form-username phoneInput3">
									<input type="hidden" name="phone"  id="phoneToString">
								</div>
								<c:set var="checkAddress" value="${user.address}" />
                     			<c:set var="address" value="${fn:split(checkAddress,'/')}" />
								<div class="form-group">
									<font style="font-size: 20px; color: white"> 주소　</font>
									<input type="text" id="sample6_postcode" value="${address[0]}" placeholder="우편번호" class="form-username form-address" >
									<input type="button" onclick="sample6_execDaumPostcode()" value="우편번호 찾기" class="btn socialForm" id="searchBtn">
								</div>
								<div class="form-group">
									<input type="text" id="sample6_address" value="${address[1]}" placeholder="주소" class="addressText">
									<div class="form-group"></div>
									<input type="text" id="sample6_address2" value="${address[2]}" placeholder="상세주소" class="addressText">
						...
						<c:forEach items="${categoryList}" var="category" varStatus="index">
							<c:choose>
								<c:when test="${category.cNO == requestScope.cList[0].cNO}">
									<input type="checkbox" name="checkbox_category"
											value="${category.cNO}"	checked="checked" class="check-form">
									<label for="c"><font style="font-size: 16px; color: white;">		
										${category.cType}
									</font></label>
								</c:when>
								...
								<c:otherwise>
									<input type="checkbox" name="checkbox_category"
											value="${category.cNO}" class="check-form">
									<label for="c"><font style="font-size: 16px; color: white;">
											${category.cType}
									</font></label>
								</c:otherwise>
							...
							<c:if test="${index.count == 3}">
								...
								<div id="category"></div>
								<button type="submit" class="btn" id="signUp">Modify Account !</button>
						</form>
						</div>
					</div>
				</div>
