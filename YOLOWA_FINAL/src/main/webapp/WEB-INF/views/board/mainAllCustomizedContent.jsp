...
<c:set var="cbListSize" value="${fn:length(cbList)}" />
<c:set var="localSize" value="0"></c:set>
<c:forEach items="${cbList}" var="cbList" varStatus="cbOrder">
	<c:if test="${cbList.fTitle eq '0'}">
		<div class="col-md-12">
			<!--여기-->
					...
					<div class="panel-heading bg-white border-none">
						<ul class="nav navbar-nav navbar-right user-nav">
							<li class="dropdown avatar-dropdown"
								onclick="bookMarkCheck('${cbList.bNo}')"><span
								class="icon-options-vertical" data-toggle="dropdown" /></span>
								<ul class="dropdown-menu user-dropdown">
									<c:choose>
										<c:when test="${user.id ==cbList.id}">
											<li><a href="modifyContentView.do?bNo=${cbList.bNo}">
													<span class="fa fa-refresh"></span> 수정
											</a></li>
											<li><a href="deleteBoard.do?bNo=${cbList.bNo}"><span
													class="fa fa-trash-o"></span> 삭제 </a></li>
										</c:when>
										<c:otherwise>
											<li><a onclick="bookMark('${cbList.bNo}')" href="#"><span
													class="" id="bookMark${cbList.bNo}"> </span></a></li>
										</c:otherwise>
									</c:choose>
								</ul></li>
						</ul>
						<h4>
							<span class="icon-notebook icons"></span> ${cbList.bType}
						</h4>
						<div style="display: inline-block; width: 25%; text-align: left;">
							작성자 : ${cbList.id}</div>
						<div style="display: inline-block; width: 30%; text-align: right;">
							등록일 : ${cbList.bPostdate}</div>
					</div>
					<hr>
					<div class="panel-body padding-0">
						<div class="col-md-12 col-xs-12 col-md-12 padding-0 box-v4-alert"
							style="background-color: #FFFFFF;">
							<pre style="background-color: #FFFFFF; border: 0px;">${cbList.bContent}</pre>
							<br>
							<c:if test="${cbList.filepath ne '0'}">
								<c:set var="checkFilePath" value="${cbList.filepath}" />
							...
							<c:if test="${cbList.local ne '0'}">
								...
								<div class="form-group row" style="margin-left: 1px;">
									...
									<div class="col-xs-2">
										<span class="fa fa-undo fa-2x" id="initRouteBtn${cbList.bNo}"
							...
							<a onclick="adminLike('${cbList.bNo}')"> <span
								class="fa fa-thumbs-o-up fa-2x" id="like${cbList.bNo}"></span></a> <span
								id="countLike${cbList.bNo}">${cbList.countlike}</span>
							&nbsp;&nbsp;&nbsp; <a><input type="hidden"
								value="${cbList.bNo}"> <span class="fa fa-angle-down"
								id="showReply${cbList.bNo}">댓글보기</span></a>
						</div>
						<!-- 댓글 -->
						<div class="col-md-12 padding-0 box-v7-comment"
							id="replyDIV${cbList.bNo}">
							<c:forEach items="${replyList}" var="rList">
								<c:if
									test="${cbList.bNo==rList.boardVO.bNo and rList.parentsNo==0}">
									<div class="media">
							...
							<div class="media">
								...
								<div class="media-body" style="text-align: center;">
									<input type="hidden" value="${cbList.bNo}">
									<textarea class="box-v7-commenttextbox"
										name="rContent${cbList.bNo}" placeholder="write comments..."
