	...
	$(document).ready(
			function() {
				$("#trashMsg").click(function() {
					...
					location.href="${pageContext.request.contextPath}/deleteMessage.do?deleteMsg="+checkboxValues+"&type=send&myId=${user.id}";
					}
					else{
						return;
					}
					...
					<tbody id="receiveMessage">
						<c:forEach items="${msgList }" var="m">
							<c:forEach items="${sendMember }" var="rId">
								<c:choose>
									<c:when test="${m.mCheck=='SM' or m.mCheck=='RD' or m.mCheck=='RM'}">
										<tr class="read">
